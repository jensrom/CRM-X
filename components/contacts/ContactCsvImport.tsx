"use client";

import { useState, useRef } from "react";
import { useRouter } from "next/navigation";
import {
  Upload, Download, CheckCircle2, AlertCircle, FileText,
  Users, X, Loader2,
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { importContactsFromCsv } from "@/app/actions/contacts";

interface Company { id: string; name: string; }

const CSV_HEADERS = [
  "fornavn",
  "efternavn",
  "email",
  "telefon",
  "mobil",
  "stilling",
  "linkedin",
  "beslutningsmandat",
  "firma",
  "noter",
];

const DECISION_ROLE_MAP: Record<string, string> = {
  "influencer":                    "influencer",
  "beslutningstagere":             "decision_maker",
  "decision_maker":                "decision_maker",
  "budgetansvarlig":               "budget_holder",
  "budget_holder":                 "budget_holder",
  "intern fortaler":               "champion",
  "champion":                      "champion",
  "ingen beslutningsmandat":       "none",
  "none":                          "none",
};

const EXAMPLE_ROWS = [
  ["Lars", "Larsen", "lars@firma.dk", "+45 12 34 56 78", "+45 87 65 43 21", "IT-chef", "https://linkedin.com/in/lars", "beslutningstagere", "Acme A/S", "Nøglekontakt"],
  ["Mette", "Madsen", "mette@beta.dk", "+45 23 45 67 89", "", "Indkøbsansvarlig", "", "budgetansvarlig", "Beta ApS", ""],
];

function downloadTemplate() {
  const lines = [CSV_HEADERS.join(";"), ...EXAMPLE_ROWS.map((r) => r.join(";"))];
  const csv = "﻿" + lines.join("\r\n");
  const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "kontakt-import-skabelon.csv";
  a.click();
  URL.revokeObjectURL(url);
}

function parseCsv(text: string): Record<string, string>[] {
  const clean = text.replace(/^﻿/, "").replace(/\r\n/g, "\n").replace(/\r/g, "\n");
  const lines = clean.split("\n").filter((l) => l.trim());
  if (lines.length < 2) return [];
  const sep = lines[0].includes(";") ? ";" : ",";
  const headers = lines[0].split(sep).map((h) => h.trim().toLowerCase());
  return lines.slice(1).map((line) => {
    const vals = line.split(sep);
    const obj: Record<string, string> = {};
    headers.forEach((h, i) => { obj[h] = (vals[i] ?? "").trim().replace(/^"|"$/g, ""); });
    return obj;
  });
}

interface PreviewRow {
  firstName: string;
  lastName: string;
  email?: string;
  phone?: string;
  mobile?: string;
  title?: string;
  linkedInUrl?: string;
  decisionRole?: string;
  companyName?: string;
  notes?: string;
  _error?: string;
}

function mapRow(row: Record<string, string>): PreviewRow {
  const firstName = row["fornavn"] || row["firstname"] || "";
  const lastName  = row["efternavn"] || row["lastname"] || "";
  if (!firstName || !lastName) {
    return { firstName, lastName, _error: "Fornavn og/eller efternavn mangler" };
  }
  const rawRole = (row["beslutningsmandat"] || row["decisionrole"] || "").toLowerCase().trim();
  const decisionRole = DECISION_ROLE_MAP[rawRole] ?? (rawRole || undefined);

  return {
    firstName,
    lastName,
    email:        row["email"] || "",
    phone:        row["telefon"] || row["phone"] || "",
    mobile:       row["mobil"] || row["mobile"] || "",
    title:        row["stilling"] || row["title"] || "",
    linkedInUrl:  row["linkedin"] || row["linkedinurl"] || "",
    decisionRole,
    companyName:  row["firma"] || row["company"] || "",
    notes:        row["noter"] || row["notes"] || "",
  };
}

const DECISION_LABELS: Record<string, string> = {
  influencer:     "Influencer",
  decision_maker: "Beslutningstager",
  budget_holder:  "Budgetansvarlig",
  champion:       "Champion",
  none:           "Ingen",
};

interface Props { companies: Company[]; }

export function ContactCsvImport({ companies }: Props) {
  const router = useRouter();
  const fileRef = useRef<HTMLInputElement>(null);
  const [preview, setPreview] = useState<PreviewRow[]>([]);
  const [fileName, setFileName] = useState("");
  const [importing, setImporting] = useState(false);
  const [result, setResult] = useState<{ ok: number; errors: string[] } | null>(null);

  function handleFile(file: File) {
    setFileName(file.name);
    setResult(null);
    const reader = new FileReader();
    reader.onload = (e) => {
      const text = e.target?.result as string;
      const rows = parseCsv(text);
      setPreview(rows.map(mapRow));
    };
    reader.readAsText(file, "UTF-8");
  }

  function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (file) handleFile(file);
  }

  function onDrop(e: React.DragEvent) {
    e.preventDefault();
    const file = e.dataTransfer.files?.[0];
    if (file) handleFile(file);
  }

  const validRows = preview.filter((r) => !r._error && r.firstName && r.lastName);
  const errorRows = preview.filter((r) => r._error);

  // Firmanavne der ikke matcher eksisterende firmaer
  const unknownCompanies = [...new Set(
    validRows
      .filter((r) => r.companyName && !companies.find((c) => c.name.toLowerCase() === r.companyName!.toLowerCase()))
      .map((r) => r.companyName!)
  )];

  async function handleImport() {
    if (!validRows.length) return;
    setImporting(true);
    try {
      const res = await importContactsFromCsv(validRows);
      setResult(res);
      if (res.ok > 0) setTimeout(() => router.push("/contacts"), 2000);
    } catch (err: any) {
      setResult({ ok: 0, errors: [err.message ?? "Ukendt fejl"] });
    } finally {
      setImporting(false);
    }
  }

  return (
    <div className="max-w-5xl space-y-6">
      <div className="bg-card border border-border rounded-xl p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h2 className="text-lg font-semibold mb-1">CSV-import af kontakter</h2>
            <p className="text-sm text-muted-foreground">
              Importer kontakter og tilknyt dem automatisk til eksisterende firmaer via firmanavnet.
            </p>
          </div>
          <Button variant="ghost" size="sm" onClick={downloadTemplate}>
            <Download className="h-4 w-4" />
            Download skabelon
          </Button>
        </div>
        <div className="p-3 bg-secondary/50 rounded-lg text-xs text-muted-foreground">
          <span className="font-medium text-foreground">Kolonner: </span>
          {CSV_HEADERS.join(" · ")}
          <span className="ml-2 text-amber-600 font-medium">* fornavn + efternavn er påkrævet</span>
        </div>
      </div>

      {/* Drop-zone */}
      {!preview.length && (
        <div
          onDrop={onDrop}
          onDragOver={(e) => e.preventDefault()}
          onClick={() => fileRef.current?.click()}
          className="border-2 border-dashed border-border hover:border-primary/50 rounded-xl p-12 flex flex-col items-center justify-center gap-4 cursor-pointer transition-colors bg-card"
        >
          <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center">
            <Upload className="h-7 w-7 text-primary" />
          </div>
          <div className="text-center">
            <p className="font-medium">Træk og slip din CSV-fil her</p>
            <p className="text-sm text-muted-foreground mt-1">eller klik for at vælge fil</p>
          </div>
          <input ref={fileRef} type="file" accept=".csv,text/csv" onChange={onFileChange} className="hidden" />
        </div>
      )}

      {/* Advarsel: ukendte firmanavne */}
      {unknownCompanies.length > 0 && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl p-4">
          <p className="text-sm text-amber-800 dark:text-amber-300 font-medium mb-1">
            {unknownCompanies.length} firmanavne fundet ikke i systemet
          </p>
          <p className="text-xs text-amber-700 dark:text-amber-400 mb-2">
            Disse kontakter importeres uden firmatilknytning. Opret firmaerne først, eller ret navnene i CSV-filen.
          </p>
          <div className="flex flex-wrap gap-1.5">
            {unknownCompanies.map((n) => (
              <span key={n} className="text-xs bg-amber-100 dark:bg-amber-800/40 text-amber-800 dark:text-amber-300 px-2 py-0.5 rounded-full">
                {n}
              </span>
            ))}
          </div>
        </div>
      )}

      {/* Preview-tabel */}
      {preview.length > 0 && (
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3.5 border-b border-border bg-secondary/30">
            <div className="flex items-center gap-3">
              <FileText className="h-4 w-4 text-muted-foreground" />
              <span className="text-sm font-medium">{fileName}</span>
              <span className="text-xs text-muted-foreground">
                {validRows.length} klar · {errorRows.length} fejl
              </span>
            </div>
            <button onClick={() => { setPreview([]); setFileName(""); setResult(null); }}
              className="text-muted-foreground hover:text-foreground transition-colors">
              <X className="h-4 w-4" />
            </button>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-xs text-muted-foreground bg-secondary/20">
                  <th className="text-left px-4 py-2.5">#</th>
                  <th className="text-left px-4 py-2.5">Navn</th>
                  <th className="text-left px-4 py-2.5 hidden sm:table-cell">Email</th>
                  <th className="text-left px-4 py-2.5 hidden md:table-cell">Telefon</th>
                  <th className="text-left px-4 py-2.5 hidden lg:table-cell">Stilling</th>
                  <th className="text-left px-4 py-2.5 hidden lg:table-cell">Firma</th>
                  <th className="text-left px-4 py-2.5 hidden xl:table-cell">Mandat</th>
                  <th className="text-left px-4 py-2.5">Status</th>
                </tr>
              </thead>
              <tbody>
                {preview.map((row, i) => (
                  <tr key={i} className={`border-b border-border/50 last:border-0 ${row._error ? "bg-destructive/5" : ""}`}>
                    <td className="px-4 py-2.5 text-muted-foreground text-xs">{i + 1}</td>
                    <td className="px-4 py-2.5 font-medium">
                      {row.firstName && row.lastName
                        ? `${row.firstName} ${row.lastName}`
                        : <span className="text-destructive text-xs">(tomt)</span>}
                    </td>
                    <td className="px-4 py-2.5 text-muted-foreground hidden sm:table-cell">{row.email || "—"}</td>
                    <td className="px-4 py-2.5 text-muted-foreground hidden md:table-cell">{row.phone || "—"}</td>
                    <td className="px-4 py-2.5 text-muted-foreground hidden lg:table-cell">{row.title || "—"}</td>
                    <td className="px-4 py-2.5 hidden lg:table-cell">
                      {row.companyName ? (
                        <span className={companies.find((c) => c.name.toLowerCase() === row.companyName!.toLowerCase())
                          ? "text-emerald-600 text-xs"
                          : "text-amber-600 text-xs"}>
                          {row.companyName}
                        </span>
                      ) : <span className="text-muted-foreground/40">—</span>}
                    </td>
                    <td className="px-4 py-2.5 hidden xl:table-cell text-xs text-muted-foreground">
                      {row.decisionRole ? DECISION_LABELS[row.decisionRole] ?? row.decisionRole : "—"}
                    </td>
                    <td className="px-4 py-2.5">
                      {row._error ? (
                        <span className="flex items-center gap-1 text-xs text-destructive">
                          <AlertCircle className="h-3 w-3" /> {row._error}
                        </span>
                      ) : (
                        <span className="flex items-center gap-1 text-xs text-emerald-600">
                          <CheckCircle2 className="h-3 w-3" /> Klar
                        </span>
                      )}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>

          <div className="px-5 py-4 border-t border-border flex items-center justify-between">
            {result ? (
              <div className={`flex items-center gap-2 text-sm ${result.ok > 0 ? "text-emerald-600" : "text-destructive"}`}>
                {result.ok > 0 ? <CheckCircle2 className="h-4 w-4" /> : <AlertCircle className="h-4 w-4" />}
                {result.ok > 0
                  ? `${result.ok} kontakter importeret! Omdirigerer...`
                  : result.errors.join(", ")}
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">
                {validRows.length} kontakt{validRows.length !== 1 ? "er" : ""} klar
                {errorRows.length > 0 && ` · ${errorRows.length} springes over`}
              </p>
            )}
            <Button
              onClick={handleImport}
              disabled={!validRows.length || importing || !!result?.ok}
              size="sm"
            >
              {importing ? (
                <><Loader2 className="h-3.5 w-3.5 animate-spin" /> Importerer...</>
              ) : (
                <><Users className="h-3.5 w-3.5" /> Importer {validRows.length} kontakter</>
              )}
            </Button>
          </div>
        </div>
      )}
    </div>
  );
}
