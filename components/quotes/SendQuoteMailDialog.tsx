...
/**
 * Send-tilbud-via-mail dialog.
 *
 * Brugeren vaelger mellem:
 *   • "Mig" — sender fra koblet personlig mailbox (M365 eller Workspace)
 *   • "System" — sender via Resend fra system-mail-adressen
 *
 * Hvis brugeren ikke har koblet sin egen mailbox, er kun "System" tilgaengelig.
 * Hvis system-mail ikke er konfigureret, vises en advarsel + link til settings.
 */
...
import { emailQuote } from "@/app/actions/quotes";
...
export function SendQuoteMailDialog({
  quoteId, defaultTo, defaultSubject, defaultMessage,
  userMailbox, systemMailbox, triggerLabel = "Send tilbud",
}: Props) {
  ...
  const handleSubmit = (formData: FormData) => {
    ...
    start(async () => {
      try {
        await emailQuote(quoteId, formData);
        setOpen(false);
      } catch (e: any) {
        ...
        >
          ...
          >
            <div className="flex items-start justify-between px-6 py-4 border-b border-border">
              <div>
                <h2 className="text-base font-semibold">Send tilbud via mail</h2>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Mailen logges i kundens aktivitetsspor.
                </p>
              </div>
              <button
                type="button"
